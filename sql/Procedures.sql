...
create or replace procedure accion_modificar_articulo(
V_NOMBRE in ARTICULOS.NOMBRE%TYPE,
V_PRECIO IN ARTICULOS.PRECIO%TYPE,
V_TALLA IN ARTICULOS.TALLA%TYPE,
V_TAGS IN ARTICULOS.TAGS%TYPE
)
IS
IDART NUMBER;
BEGIN
SELECT ARTICULOS.IDARTICULO INTO IDART FROM ARTICULOS WHERE NOMBRE = V_NOMBRE AND PRECIO = V_PRECIO AND TALLA=V_TALLA AND TAGS = V_TAGS;

UPDATE ARTICULOS SET PRECIO = V_PRECIO WHERE IDARTICULO = IDART;
UPDATE ARTICULOS SET NOMBRE = V_NOMBRE WHERE IDARTICULO = IDART;
UPDATE ARTICULOS SET TALLA = V_TALLA WHERE IDARTICULO = IDART;
UPDATE ARTICULOS SET TAGS = V_TAGS WHERE IDARTICULO = IDART;
END;
/
...
P_OID_USUARIO IN USUARIOS.OID_USUARIO%TYPE,
...
V_FECHA_NAC DATE,
V_USERNAME IN USUARIOS.NOMBREUSUARIO%TYPE) IS
BEGIN
  UPDATE Usuarios SET DNI=V_DNI WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET NOMBRE=V_NOM WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET APELLIDOS=V_APE WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET EMAIL=V_EMAIL WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET TELEFONO=V_TELF WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET DIRECCION=V_DIR WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET CONTRASEÑA=V_PASS WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET FechaNacimiento=V_FECHA_NAC WHERE OID_Usuario=P_OID_USUARIO;
  UPDATE Usuarios SET NombreUsuario=V_USERNAME WHERE OID_Usuario=P_OID_USUARIO;
END;
/

/*****************************************************************************/
/* PROCEDIMIENTO PARA OBTENER OID_USUARIO A PARTIR DEL USERNAME              */
/*****************************************************************************/

/*CREATE OR REPLACE PROCEDURE OBTENER_OID_USUARIO(
P_USERNAME IN USUARIOS.NOMBREUSUARIO%TYPE)
IS
V_OID_USUARIO IN USUARIOS.OID_USUARIO%TYPE;
BEGIN
  SELECT OID_USUARIO INTO V_OID_USUARIO FROM Usuarios WHERE NombreUsuario=P_USERNAME;
END;
/
*/
